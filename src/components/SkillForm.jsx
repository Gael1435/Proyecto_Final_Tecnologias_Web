            ...
            <form onSubmit={agregarHabilidad} className="space-y-4 mb-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div>
                        ...
                        <input id="nombre" type="text" name="nombre" value={datos.nombre} onChange={cambiarEntrada}
                    ...
                    <div>
                        <label htmlFor="categoria" className="block text-sm mb-1">
                            Categoría <span className="font-semibold">*</span>
                        </label>
                        <select id="categoria" name="categoria" value={datos.categoria} onChange={cambiarEntrada}
                            className={`w-full px-3 py-2 border rounded text-sm ${errores.categoria ? 'border-current' : 'border-gray-300 dark:border-gray-600'}`}>
                ...
                <div>
                    <label className="block text-sm mb-2">
                        Nivel <span className="font-semibold">*</span>
                    </label>

                    <div className="flex gap-3">
                        {niveles.map(nivel => (
                            <label key={nivel} className="flex items-center gap-2">
                                <input type="radio" name="nivel" value={nivel} checked={datos.nivel === nivel}
                ...
                <div>
                    ...
                    <textarea id="descripcion" name="descripcion" value={datos.descripcion} onChange={cambiarEntrada}
                    ...
                    {habilidades.map(skill => (
                        ...
                        className="flex items-start justify-between p-3 border border-gray-300 dark:border-gray-600 rounded text-sm">
                            ...
                            <div className="flex gap-2 ml-3">
                                <button
                                    onClick={() => editarHabilidad(skill)}
                                    className="text-xs"
                                >
                                    Editar
                                </button>
                                <button
                                    onClick={() => eliminarHabilidad(skill.id)}
                                    className="text-xs"
                                >
                                    Eliminar
                                </button>
